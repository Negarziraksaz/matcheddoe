...
For researchers with numeric-factor experiments, fit estimable response surfaces, inspect full model ANOVA and run influence, compare stated equations, and examine supported candidate settings. Two responses can be compared on a common grid with an explicit nondominated tradeoff set. All analysis runs locally on the computer starting the app, without an account or API key.
