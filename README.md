...
[![Scientific checks](https://github.com/Negarziraksaz/matcheddoe/actions/workflows/tests.yml/badge.svg?branch=main)](https://github.com/Negarziraksaz/matcheddoe/actions/workflows/tests.yml)

[![Download code](docs/assets/download.svg)](https://github.com/Negarziraksaz/matcheddoe/archive/refs/heads/main.zip)
[![Run locally](docs/assets/start.svg)](WEB_ACCESS.md#run-the-app-on-your-computer)
[![Example report](docs/assets/example.svg)](benchmarks/reference_run/banana_design/summary.pdf)
...
**Which response surfaces and candidate conditions are supported by the observed experimental runs?**

For researchers with numeric-factor experiments, fit estimable response surfaces, inspect full model ANOVA and run influence, compare stated equations, and examine supported candidate settings. Two responses can be compared on a common grid with an explicit nondominated tradeoff set. Analysis runs on the computer or server hosting the app, without an AI service or API key. The local launcher keeps processing on your computer; an online deployment processes uploads on its server.

```mermaid
flowchart LR
    A["Measured runs"] --> B["Common complete observations"] --> C["Rank and design checks"] --> D["Coded surface and ANOVA"] --> E["Supported candidate grid"] --> F["Surfaces, influence and tradeoffs"]
```

![Computed banana design example](benchmarks/reference_run/banana_design/primary_fit.png)
...
The [worked walkthrough](GETTING_STARTED.md) explains input preparation, missing values, supported calculations and troubleshooting. [Validation](VALIDATION.md) records the actual environments and remaining browser/CI limits. Version **0.5.2** has local verification and a recorded [successful CI run](https://github.com/Negarziraksaz/matcheddoe/actions/runs/34273166625). See [CI evidence](CI_STATUS.md) for its exact commit and scope, and [local hosting preparation](HOSTING_PREPARATION.md) for the subsequent uncommitted changes; a test pass does not establish hosting or independent scientific validation.

CSV, XLSX and pasted tables share one parser; row keys are optional. [Artificial starters](src/matcheddoe/starter_files/) explain the roles. Use [Input guide](INPUT_GUIDE.md) and [Methods](METHODS.md) for missing data and operation-specific support.
...
matcheddoe demo --output runs/first_example
matcheddoe app
```
...
```text
matcheddoe analyse --input data.csv --config config.json --output runs/my_analysis
```
...
Maintainer: [Negarziraksaz](https://github.com/Negarziraksaz). Code: MIT. Example measurements retain their original CC BY 4.0 attribution. [Development contributions](DECLARATION.md) · [Third-party notices](THIRD_PARTY_NOTICES.md) · [Contributing](CONTRIBUTING.md).
