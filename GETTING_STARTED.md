...
For local processing on Windows, install Python 3.12 or 3.14, download and extract the complete repository, and double-click `START_WINDOWS.cmd`. The first launch installs dependencies. Online uploads are processed on the hosting server; use the local installation for confidential measurements.
