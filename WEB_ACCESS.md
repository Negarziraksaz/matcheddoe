# Web access and launch — MatchedDoE
...
| [Example PDF](benchmarks/reference_run/banana_design/summary.pdf) | An actual computed report you can read without installing Python. |
| [GitHub Actions](https://github.com/Negarziraksaz/matcheddoe/actions/workflows/tests.yml) | Temporary jobs that test and build the software; they stop after completing. |
...
1. [Download the complete repository ZIP](https://github.com/Negarziraksaz/matcheddoe/archive/refs/heads/main.zip), then **Extract All**. Keep `.github`, `.streamlit`, `src` and the other folders together. A download button retrieves files; it cannot install or execute Python from the GitHub page.
...
To read the HTML example without Python, open `benchmarks/reference_run/banana_design/summary.html` from the extracted ZIP. GitHub's ordinary file view displays HTML source rather than serving it as an application.
...
3. Select **Create app**, then repository **`Negarziraksaz/matcheddoe`**, branch **`main`**, and main file **`app.py`**. In advanced settings choose **Python 3.12** if offered, matching the recorded CI major/minor route. Do not rely on an unverified default Python version. No app secret or AI API key is needed.
