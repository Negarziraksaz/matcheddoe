...
from matcheddoe.common import InputError
from matcheddoe.importing import numeric_columns, read_table
from matcheddoe.reporting import files_for_result, summary_files
from matcheddoe.templates import schema, starter_bytes, starter_frame
...
def test_optional_post_fit_failure_preserves_numerical_core(monkeypatch):
    from matcheddoe import diagnostics
...
def test_four_factors_keep_fitted_results_without_candidate_grid():
    rng = np.random.default_rng(741)
    frame = pd.DataFrame(rng.normal(size=(20, 4)), columns=list("ABCD"))
    frame["Y"] = 1 + frame.A + 2 * frame.B - 0.3 * frame.C + rng.normal(0, 0.1, 20)
    result = core.analyse(frame, factors=list("ABCD"), responses=["Y"], model="linear", id_col=None)
    assert len(result.tables["coefficients"]) == 5
    assert len(result.tables["predictions"]) == 20
    assert result.tables["candidates"].empty


def test_unavailable_secondary_response_does_not_discard_primary():
    frame = starter_frame()
    frame["Secondary"] = np.nan
    result = analyse_starter(frame, responses=["Response", "Secondary"], response_mode="per_response")
    assert set(result.tables["predictions"].response) == {"Response"}
    assert "candidate_tradeoff" not in result.tables
    assert result.status == "partial"


def test_grid_budget_disables_grid_only():
    result = analyse_starter(max_grid_candidates=2)
    assert result.tables["candidates"].empty
    assert len(result.tables["predictions"]) == 9


def test_coefficient_editor_uses_safe_numeric_schema():
    from matcheddoe.equations import coefficient_table_specification

    table = pd.DataFrame(
        {"response": ["Response", "Response"], "term": ["Intercept", "Time (min)"], "coefficient": [2, 2]}
    )
    spec = coefficient_table_specification(table, "actual")
    result = analyse_starter(reported_equations=spec)
    assert len(result.tables["equation_comparison"]) == 9
    table["coefficient"] = table.coefficient.astype(object)
    table.loc[0, "coefficient"] = "__import__('os')"
    with pytest.raises(InputError, match="finite"):
        coefficient_table_specification(table, "actual")
