...
from matcheddoe.cli import example_path, project_info
...
def test_new_upload_requires_fresh_independence_declaration():
    at = AppTest.from_file(str(APP), default_timeout=60).run()
    assert at.checkbox(key="independent_runs").value is True
    data = pd.read_csv(example_path())
    at.radio(key="data_mode").set_value("Upload a table").run()
    at.file_uploader(key="input_file").set_value(("user.csv", data.to_csv(index=False).encode(), "text/csv")).run()
    assert at.checkbox(key="independent_runs").value is False
    at.button(key="run_analysis").click().run()
    assert at.session_state["analysis_result"].tables["coefficients"].ci95_low.isna().all()
