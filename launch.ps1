...
try {
    ...
    Invoke-CheckedPython -Executable $environmentPython -Arguments @('-c','import matcheddoe, streamlit, numpy, pandas, scipy; from matcheddoe.cli import example_path; assert example_path().is_file(); print(matcheddoe.__version__)') -Failure 'A required library or packaged example is missing. Rename .venv to .venv.previous and rerun online.'
